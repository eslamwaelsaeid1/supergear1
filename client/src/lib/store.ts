...
export const Store = create<StoreType>()(
  persist(
    (set) => ({
      ...
      }, // الفلتر تقوم بانشاء مصفوفه جديده تحتوى على العناصر التي تتطابق مع الشرط المحدد
      resetCart: () => {
        set({ cartProduct: [] });
      },
      ...
      addToFavorite: (product: ProductProps) => {
        ...
        return new Promise<void> ((resolve)=>{
           set(
             (state: StoreType)=>{
                const isFavoutite = state.favoriteProduct.some((item) => item._id === product._id);
                return{
                  favoriteProduct: isFavoutite? state.favoriteProduct.filter((item) => item._id !== product._id):
      ...
      removeFromFavourite: (productId: number)=>{
        set(
          (state: StoreType)=>({
